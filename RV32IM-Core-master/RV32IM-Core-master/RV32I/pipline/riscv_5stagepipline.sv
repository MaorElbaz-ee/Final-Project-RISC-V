...
module riscv_pipline(input  logic        clk, reset,
  ...
  logic       ALUSrcD, RegWriteD, JumpD, JalrD, BranchD;
  ...
  datapath dp(clk, reset, 
              ALUSrcD, RegWriteD, MemWriteD, JumpD, BranchD, JalrD,
              ResultSrcD, 
              ImmSrcD,
              ALUControlD,
              InstrF, ReadData,
...
module controller(input  logic       funct7b5,
                  ...
                  input  logic [6:0] op,                  
                  output logic       MemWriteD, ALUSrcD, RegWriteD, JumpD, BranchD, JalrD,
  ...
  maindec md(op, 
             MemWriteD, BranchD, ALUSrcD, RegWriteD, JumpD, JalrD,
             ResultSrcD , ALUOp,
             ImmSrcD);
  aludec  ad(op[5], funct7b5, 
             ALUOp, 
             funct3, 
             ALUControlD);
...
module maindec(input  logic [6:0] op,
               output logic       MemWrite, Branch, ALUSrc, RegWrite, Jump, Jalr,
  ...
  always_comb
    ...
    // RegWrite_ImmSrc_ALUSrc_MemWrite_ResultSrc_Branch_ALUOp_Jump_Jalr
      ...
      7'b1100011: controls = 13'b0_010_0_0_xx_1_01_0_0; // B-type
      7'b1101111: controls = 13'b1_011_x_0_10_0_xx_1_0; // jal
      7'b1100111: controls = 13'b1_000_x_0_10_0_xx_1_1; // jalr
      7'b0110111: controls = 13'b1_100_1_0_00_0_00_0_0; // lui
      7'b0010111: controls = 13'b1_100_x_0_11_0_xx_0_0; // auipc
      default:    controls = 13'b0_000_0_0_00_0_00_0_0; // non-implemented instruction
    endcase
endmodule
  ...
  always_comb
    case(ALUOp)
      ...
      2'b01:   case(funct3) // Branch 
				 ...
				 3'b110, 3'b111: ALUControl = 4'b1001; // bltu, bbgeu -> sltu
...
module datapath(input  logic        clk, reset,
                input  logic        ALUSrcD, RegWriteD, MemWriteD, JumpD, BranchD, JalrD,
  ...
  assign Rs1D = TempInstrD[19:15];
  assign Rs2D = TempInstrD[24:20];
  assign InstrD = TempInstrD; 
  assign Funct3D = TempInstrD[14:12];
  
  // -------- ID/EX Register --------
  logic RegWriteE, MemWriteE, JumpE, BranchE, JalrE, ALUSrcE, FlushE;
  ...
  id_ex_stage_register id_ex(clk, reset, FlushE,
                             RegWriteD, ResultSrcD, MemWriteD, JumpD, BranchD, JalrD, ALUControlD, ALUSrcD,
                             RD1D, RD2D, PCD, RdD, Rs1D, Rs2D, ImmExtD, PCPlus4D, Funct3D,
                             RegWriteE, ResultSrcE, MemWriteE, JumpE, BranchE, JalrE, ALUControlE, ALUSrcE,
                             RD1E, RD2E, PCE, RdE, Rs1E, Rs2E, ImmExtE, PCPlus4E, Funct3E); 
  ...
  // -------- EX Stage --------
  logic [31:0] SrcAE, SrcBE, WriteDataE, ALUResultE, PCTargetE, JalTargetE, JalrTargetE, Temp_JalrTargetE;
  logic ZeroE, LTE, LTUE, TakeE;
  mux3 #(32)  fMuxA(RD1E, ResultW, ALUResultM, ForwardAE, SrcAE); 
...
module alu(input  logic [31:0] a, b,
  ...
  always_comb
    case (alucontrol)
      ...
      4'b1000:  result = lt ? 32'd1 : 32'd0;   // slt
      4'b1001:  result = ltu ? 32'd1 : 32'd0; //sltu
...
module id_ex_stage_register #(parameter WIDTH = 32) 
    ...
    floprf #(4) aluControl(clk, reset, FlushE, ALUControlD, ALUControlE);
